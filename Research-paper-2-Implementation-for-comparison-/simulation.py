...
#
#  Matrix dimension: m = 4·n (scaled representation).
#  The base paper uses m = n·k ≈ 12n with a single matrix-vector
#  product per encrypt. OO-IRIBE uses m = 4n but performs 14+
#  matrix-vector products per encrypt (c0, c_ID, 10×c'_no, c''_t),
#  giving properly heavier total computation.
# ═══════════════════════════════════════════════════════════════
...
class OO_IRIBE_System:
    ...
    def __init__(self, n, q, N_users=10, sigma=SIGMA):
        ...
        self.m = 4 * n  # scaled representation: m = 4n for proper computational weight
        self.sigma = sigma
...
# ═══════════════════════════════════════════════════════════════
#  SECTION 7: DEVICE-COUNT METRICS (20, 40, 60, 80, 100 devices)
#  Matching Comparison_Table.docx format
# ═══════════════════════════════════════════════════════════════

def run_device_metrics(device_counts=[20, 40, 60, 80, 100], n=512, q=3329):
    """
    Run device-count-scaled simulations for OO-IRIBE-EnDKER.
    ...
    for num_devices in device_counts:
        ...
        # Register a fixed pool of users (N_users=10)
        user_ids = []
        sk_data_map = {}
        for i in range(system.N_users):
            uid = f"Device_{i}"
            try:
                sk_data_map[uid] = system.gen_sk(uid)
                user_ids.append(uid)
            except ValueError:
                break
        ...
        # ---- Batch Formation: encrypt data from all num_devices devices ----
        t_batch_form_start = time.perf_counter()
        encrypted_data = []
        for i in range(num_devices):
            uid = user_ids[i % len(user_ids)]  # rotate through registered users
            ct = system.full_encrypt(uid, epoch, nrno_t, i % 2)
            encrypted_data.append(ct)
        ...
        for i in range(num_devices):
            ...
            if uid in sk_data_map:
                dk = system.gen_dk(sk_data_map[uid], epoch, nrno_t)
                no_ID = sk_data_map[uid]['no_ID']
                system.decrypt(encrypted_data[i], dk, no_ID)
        batch_decryption_time = time.perf_counter() - t_batch_dec_start

        # ---- Batch Authentication (Trust + Signatures) ----
        primary_uid = user_ids[0] if user_ids else "Device_0"
        t_batch_auth_start = time.perf_counter()
        for i in range(num_devices):
            keyword_ct = system.full_encrypt(primary_uid, epoch, nrno_t, 1)
            encrypted_keyword_bytes = keyword_ct['c0'].tobytes()
            qo = Query(encrypted_keyword=encrypted_keyword_bytes, signature=b"", epoch=epoch)
            msg = validator.serialize(primary_uid, qo)
            qo.signature = sig.sign(msg, sk_user)
            validator.validate(primary_uid, qo, pk_user)
        ...
        # ---- Storage Overhead ----
        # OO-IRIBE uses larger keys: SK has 3*m components + NL + D_no matrices
        m_used = 4 * n  # m = 4n (scaled representation)
        sk_size = 3 * m_used * 4
        ct_size = (4 * m_used + 1) * 4  # c0 + c_ID + c'_no + c''_t + C_i
        nl_overhead = system.N_users * 4
        storage_overhead_kb = (sk_size + ct_size * num_devices + nl_overhead) / 1024
...
def print_device_metrics(results):
    ...
    print("\n" + "=" * 100, flush=True)
    print("  Device-Count Metrics (OO-IRIBE-EnDKER)", flush=True)
...
if __name__ == "__main__":
    ...
    device_results = run_device_metrics(
        device_counts=[20, 40, 60, 80, 100], n=512, q=3329
    )
